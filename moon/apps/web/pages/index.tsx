...
export const getServerSideProps: GetServerSideProps = async ({ req, query }) => {
  try {
    ...
    if (organizations.length > 0) {
      ...
      // await apiClient.organizations.postJoinByToken().request('mega', req.cookies)
      // postOrganizations 创建新组织
      // const userData = await apiClient.users
      //   .getMe()
      //   .request({ headers })
      //   .then((res) => res)

      // // admin post inv to users
